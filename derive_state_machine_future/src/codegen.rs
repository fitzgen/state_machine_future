...
fn doc_string<S: AsRef<str>>(s: S) -> quote::Tokens {
    ...
    Ident::from(doc).to_tokens(&mut tokens);
    tokens
}
...
impl ToTokens for StateMachine<phases::ReadyForCodegen> {
    fn to_tokens(&self, tokens: &mut quote::Tokens) {
        ...
        let state_machine_ident = Ident::from(state_machine_name.as_str());
        ...
        let start_params = match start.fields.style {
            ...
            darling::ast::Style::Tuple => start
                ...
                .enumerate()
                .map(|(i, mut f)| {
                    f.ident = Some(syn::Ident::from(format!("arg{}", i)));
                    f
                })
                .collect(),
...
impl State<phases::ReadyForCodegen> {
    fn future_poll_match_arm(&self, ty_generics: &syn::TypeGenerics) -> quote::Tokens {
        let ident = &self.ident;
...
impl ToTokens for State<phases::ReadyForCodegen> {
    fn to_tokens(&self, tokens: &mut quote::Tokens) {
        ...
        let fields: Vec<_> = self.fields
            ...
            .map(|f| {
                let mut f = f.clone();
                f.vis = syn::parse_str("pub").unwrap();
                f
            })
            .collect();
